...
load_natural_earth_data <- function(file, dir = 'Functions/naturalearth_10m_physical', ...) {
  NAT_EARTH_PATH <- 'Functions/naturalearth_10m_physical'
...
STACYmap_discrete <- function(gridlyr = NULL, 
                     ptlyr = NULL, 
                     ptlyr_size_var = NULL,
                     ...
                     box = TRUE, 
                     limits_color = NULL,
                     labels_discrete = NULL,
                     discrete_breaks = NULL) {
  ...
  if (!is.null(centercolor)) {
    ...
    if(!is.null(limits_color)){
      lims <- list(grid = c(limits_color$min, limits_color$max), pt = c(limits_color$min, limits_color$max))
    }else if (!splcol & !is.null(gridlyro) & !is.null(ptlyro)) {
    ...
    if(!is.null(limits_color)){
      lims <- list(grid = c(limits_color$min, limits_color$max))
    }else{
      lims <- list(grid = c(min(gridlyr$layer), max(gridlyr$layer)))
    }
  } else if (!is.null(ptlyr)) {
    if(!is.null(limits_color)){
      lims <- list(grid = c(limits_color$min, limits_color$max))
    }else{
      lims <- list(grid = c(min(ptlyr$layer), max(ptlyr$layer)))
    }
  }
  ...
  if (!is.null(gridlyr)) {
    ...
    if(!legend_cb){
      # map_plot <- map_plot + scale_fill_stepsn(colors = {if (splcol) {colorscheme$grid} else {colorscheme}},
      #                                             limits = lims$grid,
      ...
      #   scale_color_stepsn(colors = {if (splcol) {colorscheme$grid} else {colorscheme}}, 
      #                         limits = lims$grid, guide = FALSE) 
      
      
    } else {
      map_plot <- map_plot + scale_fill_stepsn(colors = {if (splcol) {colorscheme$grid} else {colorscheme}},
                                               limits = lims$grid,breaks = discrete_breaks,labels = labels_discrete,
                                               guide = clrgd(legend_names$grid,
                                                             breaks = discrete_breaks)) +
        scale_color_stepsn(colors = {if (splcol) {colorscheme$grid} else {colorscheme}}, 
                           limits = lims$grid, breaks = discrete_breaks, guide = FALSE)
    } 
  ...
  if (!is.null(ptlyr)) {
    ...
    if (splcol | is.null(gridlyr)) {
      map_plot <- map_plot + 
        scale_fill_stepsn(colors = {if (splcol) {colorscheme$pt} else {colorscheme}}, 
                          limits = lims$point, 
                          breaks = discrete_breaks, labels = labels_discrete,
